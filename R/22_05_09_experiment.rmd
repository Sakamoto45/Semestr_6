...
normalize <- function(signal, norm = 1, len = length(signal)) signal / mean(signal[1:len]) * norm

MSE <- function(a, b) mean((a - b)^2)


exp_ <- function(base, len = 150) exp(base * (1:len))
cos_ <- function(period, phase = 0, len = 150) cospi(2 * ((1:len) / period + phase))


repeats <- 1

# придумать, как нормировать сигналы

# exps <- lapply(c(0, 0.005, 0.015, 0.025), exp_)
# signals <- exps

signals <- mapply(rep, c(1, 2, 3, 4), rep(150, 4), SIMPLIFY = FALSE)
noises <- list(
    lapply(rep(150, repeats), rnorm),
    lapply(rep(150, repeats), rnorm))

# noises[[1]][[3]]
# noises1 <- lapply(rep(150, repeats), rnorm)
# noises2 <- lapply(rep(150, repeats), rnorm)
# rnorm(len, mean, sd)

# get_signal1 <- function(x) signals[[x$signal_id1]] * noises[x$noise_id, 1]

# get_signal2 <- function(x) signals[[x$signal_id2]] * noises[x$noise_id, 2]



ssa_error_reconstruct <- function(x) {
    signal <- (signals[[x$signal_id1]] * noises[[1]][[x$noise_id]])[1:x$l_known]
    return(
        MSE(signals[[x$signal_id1]][1:x$l_known],
        reconstruct(ssa(signal, L = x$l_window),
            groups = x$group)))
    # print("ssa_error_reconstruct called")

}

ssa_error_forecast <- function(x) {
    signal <- (signals[[x$signal_id1]] * noises[[1]][[x$noise_id]])[1:x$l_known]
    return(
        MSE(signals[[x$signal_id1]][x$l_known + (1:x$l_forecast)],
        rforecast(ssa(signal, L = x$l_window),
            groups = x$group, len = x$l_forecast)))
    # print("ssa_error_forecast called")
}
...
df <- df %>%
    group_by(signal_id1, noise_id, l_known, l_forecast, l_window, group)# %>%
    # mutate(
    #     ssa_error_reconstruct = ssa_error_reconstruct(slice(1)),
    #     ssa_error_forecast = ssa_error_forecast(slice(1))) %>%
    # ungroup()
