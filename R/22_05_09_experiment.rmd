...
# l_fcst - length of forecast
# l_win - length of window
# repeats
...
ssa_error_recon <- function(x) {
    # if (is.na(x$l_win)) x$l_win <- (x$l_known + 1) %/% 2
    # signal_known <- signals[[x$signal1]] * noises1[[x$noise1]]


    # print("ssa_error_fcst called")
    # rforecast(ssa())
    return(1)
}

ssa_error_fcst <- function(x) {
    # if (is.na(x$l_win)) x$l_win <- (x$l_known + 1) %/% 2
    # signal_known <- signals[[x$signal1]] * noises[[x$noise1]]
...
df <- df %>%
    group_by(signal_id1, noise_id, l_known, l_fcst, l_win, group) %>%
    mutate(
        ssa_error_recon = ssa_error_recon(slice(1)),
        ssa_error_fcst = ssa_error_fcst(slice(1))) %>%
    ungroup()
