...
parse_vehicle <- function(vehicle, id) {
    ...
    if (is.character(failures) & length(failures) > 0) {
        failures <- data.frame(
            failure = factor(str_wrap(failures, 60)),
            vehicle_id = id
        )
    }
...
ggplot() +
    geom_line(data = accidents %>%
        mutate(timestamp = format(timestamp, format = "%Y-%m-%d")) %>%
        group_by(timestamp) %>%
        count() %>%
        ungroup(),
        aes(x = timestamp, y = n, group = 1)) +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
    labs(title = "Зависимость кол-ва аварий от даты",
...
ggplot() +
    geom_line(data = accidents %>%
        ...
        aes(x = timestamp, y = n, group = 1)) +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
    labs(title = "Зависимость кол-ва аварий от месяца",
    x = "Дата", y = "Количество аварий")
