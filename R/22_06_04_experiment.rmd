...
```{r}
pdf(file = "experiment_3_secondpart.pdf")
ggplot(df) +
    geom_path(aes(x = signal_slope,
...
ggplot(df,
    ...
    geom_abline(intercept = -2, slope = 1/4, linetype = 2) +
    labs(title = "At which noise the second component is lost",
         x = "log10 noise variance",
...
Если это линейный график, y = x/4 - 2 (пунктирная линия) то дисперсия ошибки при которой теряется вторая компонента пропорциональна четвертой степени наклона сигнала т.е.  noise_variance * 100 = signal_slope.
